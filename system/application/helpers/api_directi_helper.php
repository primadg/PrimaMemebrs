<?php
/**
 * Helper for API LogicBoxes (DirectI) 
 */
	...
	*/
	...
	function getResponse(){
	  ...
	  if ($this->scheme == 'http') {
		...
		foreach ($header_array as $header_line) {
			$arr = explode(': ',$header_line);
			$this->incoming_headers[strtolower(trim($arr[0]))] = isset($arr[1]) ? trim($arr[1]) :"";
// kgg
//$arr_e = $this->incoming_headers;
//			fb($arr_e, "api arr[1]");			
		}
		if (strlen($data) == 0) {
...
class wsdl extends XMLSchema {
	...
	var $proxyusername = '';
	var $proxypassword = '';
	// for change to debug and not debug mode
//    var $serviceurl="http://api.onlyfordemo.net/anacreon/servlet/APIv3";
    var $serviceurl="";
     ...
     * @access public 
     */
    function wsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$serviceurl=""){
        $this->wsdl = $wsdl;
...
//					        global $serviceurl;   // This line is added By Manish.
					        $this->ports[$this->currentPort]['location'] = $this->serviceurl;   //This line is added By Manish
...
//					        $this->bindings[ $this->ports[$this->currentPort]['binding'] ]['endpoint'] = $attrs['location']; //This line is commented By Manish
					        $this->bindings[ $this->ports[$this->currentPort]['binding'] ]['endpoint'] = $this->serviceurl; //This line is added By Manish
//fb($this->serviceurl,__FUNCTION__ . " serviceurl");
					        break;
					} 
					break;
			} 
...
* $soapclient = new  soapclientMy( string path [ ,boolean wsdl] );
*
...
* @access   public
*/
class  soapclientMy extends nusoap_base  {
...
//    var $serviceurl="http://api.onlyfordemo.net/anacreon/servlet/APIv3";
	...
	* @access   public
	*/
	function  soapclientMy($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false,$serviceurl=""){
		...
		$this->proxypassword = $proxypassword;
		$this->serviceurl = $serviceurl;
//fb($this->serviceurl,"soapclientMy serviceurl");
		// make values
		if($wsdl){
			...
			$this->wsdl =& new wsdl($this->wsdlFile,$this->proxyhost,$this->proxyport,$this->proxyusername,$this->proxypassword,$this->serviceurl);
			$this->debug("wsdl debug: \n".$this->wsdl->debug_str);
			$this->wsdl->debug_str = '';
			$this->wsdl->serviceurl = $this->serviceurl;
			// catch errors
